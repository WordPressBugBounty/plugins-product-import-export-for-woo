...
if ( ! class_exists( 'Wbte_Accessibility_Banner' ) ) {
	...
	class Wbte_Accessibility_Banner {
		 ...
		 * @var string
		 */
		private static $plugin_prefix = 'wt_p_iew';
		...
		public static function install_plugin() {
			try {
				...
				$response = array(
					'status'  => false,
					'message' => esc_html__( 'Something went wrong. Please try again.', 'product-import-export-for-woo' ),
				);

				if ( ! current_user_can( 'install_plugins' ) ) {
					$response['message'] = esc_html__( 'You do not have sufficient permissions to install plugins.', 'product-import-export-for-woo' );
				...
				if ( ! isset( $api->download_link ) ) {
					$response['message'] = esc_html__( 'Plugin download link not found.', 'product-import-export-for-woo' );
					wp_send_json( $response );
					return;
				}
		...
		public static function activate_plugin() {
			...
			if ( ! current_user_can( 'activate_plugins' ) ) {
				$result['msg'] = esc_html__( 'You do not have sufficient permissions to activate plugins.', 'product-import-export-for-woo' );
				return $result;
			}
			...
			if ( file_exists( WP_PLUGIN_DIR . '/' . self::$plugin_file ) ) {
				...
				if ( is_wp_error( $activation_result ) ) {
					$result['status'] = false;
					$result['msg']    = esc_html__( 'Activation failed. Please try again.', 'product-import-export-for-woo' );
				} else {
					$result['status'] = true;
				}
			}
		...
		public function show_banner_notice() {
			...
			<div id="wbte_accessibility_banner" class="notice">
				<div class="wbte_accessibility_banner_main">
					<img src="<?php echo esc_url( plugin_dir_url( __FILE__ ) . 'assets/images/accessyes.svg' ); ?>" alt="<?php esc_attr_e( 'Accessibility Widget', 'product-import-export-for-woo' ); ?>" style="width: 48px; height: 48px;">
					<div class="wbte_accessibility_banner_content">
						...
						<p style="font-size: 14px;">
							<?php
							esc_html_e( 'A lightweight accessibility widget to improve accessibility and usability for every users. Set it up in minutes for free.', 'product-import-export-for-woo' );
							?>
						</p>
					</div>
				</div>
				<div class="wbte_accessibility_banner_actions">
					<button class="button button-primary" id="wbte_accessibility_banner_install_btn">
						...
						<span class="wbte_accessibility_banner_install_btn_text">
							<?php esc_html_e( 'Install & activate', 'product-import-export-for-woo' ); ?>
						</span>
					</button>
					<a href="#" class="wbte_accessibility_banner_maybe_ltr">
						<?php esc_html_e( 'Remind me later', 'product-import-export-for-woo' ); ?>
					</a>
				</div>
				<span class="wbte_accessibility_banner_dismiss_btn">
					<?php esc_html_e( 'Dismiss', 'product-import-export-for-woo' ); ?>
		...
		public static function enqueue_admin_scripts() {
			...
			$error_msg = esc_js( __( 'Something went wrong. Please try again.', 'product-import-export-for-woo' ) );
			?>
			<style>
			...
			( function( $ ) {
				...
				var wbteAccessyesNotify = {
					success: function( message ) {
						...
						} else if ( typeof wbte_pimpexp_notify_msg !== 'undefined' ) {
							wbte_pimpexp_notify_msg.success( message );
						} else {
					...
					error: function( message ) {
						if ( typeof wbte_sc_notify_msg !== 'undefined' ) {
							...
							wbte_pimpexp_notify_msg.error( message );
						} else {
							window.alert( message );
						}
					}
				...
				$( document ).ready( function() {
					...
					function setButtonLoading( isLoading ) {
						if ( isLoading ) {
							...
							btnTextSpan.text( '<?php echo esc_js( __( 'Installing...', 'product-import-export-for-woo' ) ); ?>' );
						...
						$.ajax( {
							...
							success: function( response ) {
								if ( response.status ) {
									...
									wbteAccessyesNotify.success( '<?php echo esc_js( __( 'Plugin installed successfully.', 'product-import-export-for-woo' ) ); ?>' );
								} else {
									setButtonLoading( false );
